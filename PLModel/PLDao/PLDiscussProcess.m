...
{
    NSString *code = [BLTool getKeyCode:[NSString stringWithFormat:@"%d%d%@%@",pageSize, currentPage, courseId, @"0"]];
    NSString *url = [NSString stringWithFormat:@"%d/%d/%@/%@/%@",pageSize, currentPage, courseId,@"0", code];
...
-(void) launchCourseDiscuss:(NSString *)courseId didUserId:(NSString *)userId didContent:(NSString *)discussContent didSuccess:(CallBackBlockSuccess)success didFail:(CallBackBlockFail)fail
{
    ...
    NSString *code = [BLTool getKeyCode:[NSString stringWithFormat:@"%@%@%@%@",courseId, userId, encoderContent,@"0"]];
...
{
    NSString *encoderContent = [BLTool getEncoding:discussContent];
    NSString *code = [BLTool getKeyCode:[NSString stringWithFormat:@"%@%@%@%@",workId, userId, encoderContent,@"1"]];
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] initWithCapacity:0];
...
-(void) replyDiscuss:(NSString *)userId didDiscuss:(NSString *)discussId didContent:(NSString *)content didSuccess:(CallBackBlockSuccess)success didFail:(CallBackBlockFail)fail
{
    ...
    NSString *code = [BLTool getKeyCode:[NSString stringWithFormat:@"%@%@%@%@",userId, discussId, content,@"0"]];
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] initWithCapacity:0];
    [dic setObject:userId forKey:@"userId"];
